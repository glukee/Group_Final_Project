...
public class GameLogic {
    ...
    private static boolean squaresAreInvalid(int[][] grid) {
        if(rowOfSquaresIsInvaid(Rows.TOP, grid)) return true;

        if(rowOfSquaresIsInvaid(Rows.MIDDLE, grid)) return true;

        if(rowOfSquaresIsInvaid(Rows.BOTTOM, grid)) return true;
        return false;
    }

    private static boolean rowOfSquaresIsInvaid(Rows value, int[][] grid) {
        switch (value) {
            case TOP:
